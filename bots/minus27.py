...
def get_buy_or_sell_signal(data):
    ...
    # if current_candle['volume'].item() > np.std(data[-50:]['volume']) + np.mean(data[-50:]['volume']):
    #     return 'buy'
    
  

    if (previous_candle['close'] > current_candle['open']) and (current_candle['close'] > previous_candle['open']) and (current_candle['volume'] > (np.mean(data[-50:]['volume'])+np.std(data[-50:]['volume']))):
        return 'buy'

    if current_candle['volume'].item() < np.mean(data[-50:]['volume']) - np.std(data[-50:]['volume']):
        return 'sell'
